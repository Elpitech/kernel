				   ...
				   struct drm_crtc_state *state)
{
	...
	unsigned long rate, available_rate;
	...
	if (available_rate > (rate - rate / 128) &&
		...
		return 0;
	DRM_ERROR("Requested pixel clock is %lu Hz, available - %lu\n",
		  rate, available_rate);
	return -EINVAL;
}
			   ...
			   const struct drm_display_mode *mode)
{
	...
	*vpos = p / hsize;
	if (*vpos >= mode->vdisplay || in_vblank) {
		dev_dbg(dev->dev, "vpos = %d, vdisplay = %d\n", *vpos, mode->vdisplay);
		dev_dbg(dev->dev, "DBAR %x, DCAR %x, MRR %x, ISR %x, bpp %d\n",
