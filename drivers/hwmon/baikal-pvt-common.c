...
static inline u32 pvt_update(u32 pvt_base, u32 offset, u32 mask, u32 data)
{
	u32 old;
...
static inline void pvt_set_trim(struct pvt_hwmon *pvt, u32 trim)
{
	...
	old = pvt_update(pvt->base, PVT_CTRL, PVT_CTRL_EN, 0);
	pvt_update(pvt->base, PVT_CTRL, PVT_CTRL_TRIM_MASK | PVT_CTRL_EN,
		   trim | old);
}
...
static long read_data_from_dev(struct pvt_hwmon *pvt, enum pvt_sensor_type type)
{
	long val;
	u32 data;
	int ret;
	...
	if (type == PVT_TEMP)
		val = polynomial_calc(get_polynomial_N_to_temp(), data);
	else
		val = polynomial_calc(get_polynomial_N_to_volt(), data);
...
static irqreturn_t pvt_soft_isr(int irq, void *data)
{
	...
	old = pvt_update(pvt->base, PVT_INTR_MASK, PVT_INTR_DVALID, PVT_INTR_DVALID);
	...
	if (thres_sts & info->thres_sts_lo) {
		if (type == PVT_TEMP)
			value = polynomial_calc(get_polynomial_N_to_temp(), cache->data);
		else
			value = polynomial_calc(get_polynomial_N_to_volt(), cache->data);

		pr_notice("ALARM LOW %s,\tvalue: %ld\tpvt->base: %x\n ",
			  info->label, value, pvt->base);
		pr_notice("call kernel_halt()");
		kernel_halt();
	} else if (thres_sts & info->thres_sts_hi) {
		if (type == PVT_TEMP)
			value = polynomial_calc(get_polynomial_N_to_temp(), cache->data);
		else
			value = polynomial_calc(get_polynomial_N_to_volt(), cache->data);

		pr_notice("ALARM HIGH %s,\tvalue: %ld\tpvt->base: %x\n ",
			  info->label, value, pvt->base);
		pr_notice("call kernel_halt()");
		kernel_halt();
	}
			 ...
			 long *val)
{
	...
	if (is_interrupt_present(pvt) != 1) {
		*val = read_data_from_dev(pvt, type);
		return 0;
	}
			   ...
			   bool is_low, long val)
{
	...
	if (type == PVT_TEMP) {
		...
		data = polynomial_calc(get_polynomial_temp_to_N(), val);
	} else {
		val = clamp(val, PVT_VOLT_MIN, PVT_VOLT_MAX);
		data = polynomial_calc(get_polynomial_volt_to_N(), val);
	}
	...
	limit = readl_pvt(pvt->base, pvt_info[type].thres_base);
	if (is_low) {
	...
	pvt_update(pvt->base,  pvt_info[type].thres_base, mask, data);
...
static irqreturn_t pvt_hard_isr(int irq, void *data)
{
	...
	pvt_update(pvt->base, PVT_INTR_MASK, PVT_INTR_DVALID, PVT_INTR_DVALID);
	/*
			 ...
			 long *val)
{
	 ...
	 * timeout by the factor of ten to prevent a false timeout.
	 */

	timeout = 10 * usecs_to_jiffies(ktime_to_us(pvt->timeout));
...
{
	/*
	 * While clocks are disabled use this stumb for timeout value.
	 * See full implementation in pvt driver from BE-T
	 */
	return 0;
}
...
static int pvt_check_pwr(struct pvt_hwmon *pvt)
{
	...
	pvt_update(pvt->base, PVT_INTR_MASK, PVT_INTR_ALL, PVT_INTR_ALL);
	pvt_update(pvt->base, PVT_CTRL, PVT_CTRL_EN, PVT_CTRL_EN);
	pvt_set_tout(pvt, 0);
	readl_pvt(pvt->base, PVT_DATA);
...
static int pvt_set_limits(struct pvt_hwmon *pvt)
{
	int ret = 0;
#ifdef CONFIG_SENSORS_BT1_PVT_ALARMS
	int is_low = 1;

	if (is_interrupt_present(pvt) != 1)
		return 0;

	ret |= pvt_write_limit(pvt, PVT_TEMP,  is_low, PVT_THRES_TEMP_MIN);
	ret |= pvt_write_limit(pvt, PVT_TEMP, !is_low, PVT_THRES_TEMP_MAX);

	ret |= pvt_write_limit(pvt, PVT_VOLT,  is_low, PVT_THRES_VOLT_MIN);
	ret |= pvt_write_limit(pvt, PVT_VOLT, !is_low, PVT_THRES_VOLT_MAX);

	ret |= pvt_write_limit(pvt, PVT_LVT,  is_low, PVT_THRES_VOLT_MIN);
	ret |= pvt_write_limit(pvt, PVT_LVT, !is_low, PVT_THRES_VOLT_MAX);

	ret |= pvt_write_limit(pvt, PVT_HVT,  is_low, PVT_THRES_VOLT_MIN);
	ret |= pvt_write_limit(pvt, PVT_HVT, !is_low, PVT_THRES_VOLT_MAX);

	ret |= pvt_write_limit(pvt, PVT_SVT,  is_low, PVT_THRES_VOLT_MIN);
	ret |= pvt_write_limit(pvt, PVT_SVT, !is_low, PVT_THRES_VOLT_MAX);
#endif
	return ret;
}
...
static int pvt_init_iface(struct pvt_hwmon *pvt)
{
	...
	/*
	 * While clocks are disabled this fragment is commented out
	 * See full implementation in pvt driver from BE-T
	 */

	/*
	 * unsigned long rate = clk_get_rate(pvt->clks[PVT_CLOCK_REF].clk);
	 * if (!rate) {
	...
	pvt_update(pvt->base, PVT_INTR_MASK, PVT_INTR_ALL, PVT_INTR_ALL);
	pvt_update(pvt->base, PVT_CTRL, PVT_CTRL_EN, 0);
	readl_pvt(pvt->base, PVT_CLR_INTR);
	readl_pvt(pvt->base, PVT_DATA);
...
int pvt_probe(struct platform_device *pdev)
{
	struct pvt_hwmon *pvt;
	int ret;
